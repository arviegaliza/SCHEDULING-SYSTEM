...
function Schedule() {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      const res = await axios.get('http://192.168.1.21:8081/api/events');
  ...
  const fetchDepartments = useCallback(async () => {
    try {
      const res = await axios.get('http://192.168.1.21:8081/api/department');
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const res = await axios.get('http://192.168.1.21:8081/api/categories');
  ...
  const handleAddOrUpdate = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://localhost:8081/api/events/${editingId}`, payload);
